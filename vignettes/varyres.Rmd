...
The `cutoff` argument gives the user control over maximum box sample size. For example, `cutoff = 100` means each box will have at most 100 observations, which gives this map.
...
The `max` argument specifices a maximum number of iterations. For example, if you accidentally specify `cutoff = 0` the algorithm would never stop without `max = 6` as the default.

You may want to add box sample size to your plot, and it's pretty easy to do with the data stored in `vr` and `geom_text()`.
